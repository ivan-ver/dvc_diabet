...
df = pd.get_dummies(df, columns=['gender', 'hypertension', 'heart_disease', 'smoking_history'], dtype=int)
...
]).fit_transform(df[std_columns])

for i, col_name in enumerate(std_columns):
    df[col_name] = pipline_res[:, i]


test, train =  train_test_split(df, train_size=params['split'], random_state=params['seed'])
